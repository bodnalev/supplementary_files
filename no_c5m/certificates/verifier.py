...
def recover_from_certificate(file_name):
    ...
    for ftype in types:
        same_type_flags = [flag(tflag[0],
                                ...
                                list(tflag[1]),
                                [list(x) for x in (tflag[2][1][1]
                                                   if len(tflag[2]) == 2 else [])],
                                [x[0] for x in (tflag[2][2][1] if len(tflag[2]) == 4 else [])],
                                [x[0] for x in (tflag[2][3][1] if len(tflag[2]) == 4 else [])]
                                )
...
for index in range(start - 1, end):
    ...
    if verbose >= 2:
        print("Fixed base graph: size: %d edges: %s ftype: %s parts: %s col2: %s col3: %s" %
              (F.size, str(F.edges), str(F.ftype), str(F.parts), str(list(F.col2)), str(list(F.col3))))
